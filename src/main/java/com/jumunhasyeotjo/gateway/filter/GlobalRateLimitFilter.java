        ...
        return captureRequest(exchange)
                ...
                .flatMap(response -> {
                    ...
                    if (response.isQueued()) {
                        log.info("Request queued: path={}, position={}, queueType={}",
                                path, response.getQueuePosition(), response.getQueueType());
                        return respondQueued(exchange, response);
                    }

                    log.warn("Request rejected: {}", path);
